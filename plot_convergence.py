...
def load_snap_clouds(path, dataset, only_last=False):
    ...
    decoded_path = [f.decode('UTF-8') for f in listdir(path)]
    cloud_folders = np.array([join(path, f) for f in decoded_path if f.startswith('val_preds')])
...
def compare_trainings(list_of_paths, list_of_labels=None):
    ...
    for path in list_of_paths:
        ...
        # tmp = [f.decode('UTF-8') for f in listdir(path)]
        # print(type(tmp[1]))
        # print('val_IoUs.txt' in tmp)

        # bIOUS = bytes('val_IoUs.txt', 'utf-8')
        # bCONF = bytes('val_confs.txt', 'utf-8')
        # print(bIOUS, bCONF)
        # tmp = [f for f in listdir(path)]
        # print(tmp)
        # print(bIOUS in tmp)

        if ('val_IoUs.txt' in [f.decode('UTF-8') for f in listdir(path)]) or ('val_confs.txt' in [f.decode('UTF-8') for f in listdir(path)]):
            config = Config()
            config.load(path)
        else:
            continue
...
def compare_convergences_segment(dataset, list_of_paths, list_of_names=None):
    ...
    # print(snap_epochs)  # epoch 250 and epoch 500
    print(10*'-' + '|' + 10*config.num_classes*'-')
