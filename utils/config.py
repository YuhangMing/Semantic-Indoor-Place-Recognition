...
class Config:
    ...
    def print_current(self):
        ...
        print('# ****************')
        print('dataset = {:s}'.format(self.dataset))
        print('dataset_task = {:s}'.format(self.dataset_task))
        if type(self.num_classes) is list:
            ...
            print('num_classes = {:d}'.format(self.num_classes))
        print('in_points_dim = {:d}'.format(self.in_points_dim))
        print('in_features_dim = {:d}'.format(self.in_features_dim))
        print('in_radius = {:.6f}'.format(self.in_radius))
        print('input_threads = {:d}'.format(self.input_threads))
        ...
        for a in self.architecture:
            if a in ['resnetb_strided', 'resnetb_deformable_strided', 'nearest_upsample']:
                print('\n',' '*13, '{:s}'.format(a), end=' ')
            else:
                print('{:s}'.format(a), end=' ')
        print('')
        print('num_layers = {:d}'.format(self.num_layers))
        print('equivar_mode = {:s}'.format(self.equivar_mode))
        print('invar_mode = {:s}'.format(self.invar_mode))
        print('first_features_dim = {:d}'.format(self.first_features_dim))
        print('use_batch_norm = {:d}'.format(int(self.use_batch_norm)))
        print('batch_norm_momentum = {:.6f}'.format(self.batch_norm_momentum))
        print('segmentation_ratio = {:.6f}'.format(self.segmentation_ratio))

        # KPConv parameters
        print('# KPConv parameters')
        print('# *****************')
        print('num_kernel_points = {:d}'.format(self.num_kernel_points))
        print('first_subsampling_dl = {:.6f}'.format(self.first_subsampling_dl))
        print('conv_radius = {:.6f}'.format(self.conv_radius))
        print('deform_radius = {:.6f}'.format(self.deform_radius))
        print('KP_extent = {:.6f}'.format(self.KP_extent))
        print('KP_influence = {:s}'.format(self.KP_influence))
        print('aggregation_mode = {:s}'.format(self.aggregation_mode))
        print('fixed_kernel_points = {:s}'.format(self.fixed_kernel_points))
        print('modulated = {:d}'.format(int(self.modulated)))
        print('n_frames = {:d}'.format(self.n_frames))
        print('max_in_points = {:d}'.format(self.max_in_points))
        print('max_val_points = {:d}'.format(self.max_val_points))
        print('val_radius = {:.6f}'.format(self.val_radius))

        # Training parameters
        print('# Training parameters')
        print('# *******************')
        print('learning_rate = {:f}'.format(self.learning_rate))
        print('momentum = {:f}'.format(self.momentum))
        print('lr_decay_epochs = [hide for better visual]')
        # for e, d in self.lr_decays.items():
        #     print(' {:d}:{:f}'.format(e, d))
        # print('')
        print('grad_clip_norm = {:f}'.format(self.grad_clip_norm))
        print('augment_scale_anisotropic = {:d}'.format(int(self.augment_scale_anisotropic)))
        print('augment_scale_min = {:.6f}'.format(self.augment_scale_min))
        print('augment_scale_max = {:.6f}'.format(self.augment_scale_max))
        print('augment_symmetries =', end=' ')
        for a in self.augment_symmetries:
            print('{:d}'.format(int(a)), end=' ')
        print('')
        print('augment_rotation = {:s}'.format(self.augment_rotation))
        print('augment_noise = {:f}'.format(self.augment_noise))
        print('augment_color = {:.6f}'.format(self.augment_color))
        print('augment_occlusion = {:s}'.format(self.augment_occlusion))
        print('augment_occlusion_ratio = {:.6f}'.format(self.augment_occlusion_ratio))
        print('augment_occlusion_num = {:d}'.format(self.augment_occlusion_num))
        print('weight_decay = {:f}'.format(self.weight_decay))
        print('segloss_balance = {:s}'.format(self.segloss_balance))
        print('class_w =', end=' ')
        for a in self.class_w:
            print('{:.6f}'.format(a), end=' ')
        print('')
        print('deform_fitting_mode = {:s}'.format(self.deform_fitting_mode))
        print('deform_fitting_power = {:.6f}'.format(self.deform_fitting_power))
        print('deform_lr_factor = {:.6f}'.format(self.deform_lr_factor))
        print('repulse_extent = {:.6f}'.format(self.repulse_extent))
        print('batch_num = {:d}'.format(self.batch_num))
        print('val_batch_num = {:d}'.format(self.val_batch_num))
        print('max_epoch = {:d}'.format(self.max_epoch))
        if self.epoch_steps is None:
            print('epoch_steps = None')
        else:
            print('epoch_steps = {:d}'.format(self.epoch_steps))
        print('validation_size = {:d}'.format(self.validation_size))
