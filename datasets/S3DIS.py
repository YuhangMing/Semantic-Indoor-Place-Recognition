...
class S3DISDataset(PointCloudDataset):
    ...
    def __init__(self, config, set='training', use_potentials=True, load_data=True):
        ...
        # Prepare ply files
        ###################
        # One ply file for each point cloud (area)
        if self.set == 'visualise':
            self.room_name = 'office_7'
        ...
        # get a list of ply file names (absolute path)
        self.files = []
        for i, f in enumerate(self.cloud_names):
            if self.set == 'training':
                ...
                self.files += [join(ply_path, 'Area_5', self.room_name + '.ply')]
                break
            else:
                raise ValueError('Unknown set for S3DIS data: ', self.set)
        print(self.files)
        ...
        if use_potentials:
            ...
            for i, tree in enumerate(self.pot_trees):
                ...
                # so here, generated an 1D array of size coarser_point_num of
    ...
    def potential_item(self, batch_i, debug_workers=False):
        # !!!!!!! got the idea but still fuzzy about the detailed implementation !!!!!!!
        # !!!!!!! what exactly is this potential stands for !!!!!!!
        ...
        while True:
            ...
            # Get original height as additional feature !!!!!!! PROBLEM with using height
    ...
    def load_subsampled_clouds(self):
        ...
        for i, file_path in enumerate(self.files):
            ...
            if exists(KDTree_file):
                ...
                sub_labels = np.squeeze(sub_labels)     # eg. from shape (1, 3, 1) to shape (3,), axis to be squeezed out must have size 1
        ...
        if self.use_potentials:
            ...
            for i, file_path in enumerate(self.files):
                ...
                if exists(coarse_KDTree_file):
                    ...
                    # the attay is not copied unless this is necessary
