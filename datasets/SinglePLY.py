...
# ----------------------------------------------------------------------------------------------------------------------
#
#      Class handling single ply input file.
#      TEST ONLY
...
class SinglePlyDataset(PointCloudDataset):
    """Class to handle S3DIS dataset."""

    def __init__(self, config, set='test', use_potentials=True, load_data=True):
        """
        This dataset is small enough to be stored in-memory, so load all point clouds here
        """
        PointCloudDataset.__init__(self, 'SinglePLY')

        ############
        # Parameters
        ############
        ## Using labels and color from S3DIS dataset
        ...
        # Seven Scenes
        # self.cloud_names = ['chess', 'fire', 'heads', 'office', 'pumpkin', 'redkitchen', 'stairs']
        self.cloud_names = ['pumpkin']
        self.all_splits = [0]
        ...
        if self.set in ['test', 'ERF', 'visualise']:
            ...
            raise ValueError('Unknown set for: ', self.set)
        ...
        ###################
        # # S3DIS
        # self.room_name = 'office_17'
        # # self.prepare_S3DIS_ply()
        # if self.set == 'visualise':
        #     self.prepare_room_ply('Area_5')
        # Seven Scenes
        self.room_name = 'point_cloud_880'
        # self.prepare_S3DIS_ply()
        if self.set == 'visualise':
            self.prepare_room_ply('pumpkin')    
        ...
        for i, f in enumerate(self.cloud_names):
            ...
            elif self.set == 'visualise':
                if self.all_splits[i] == self.validation_split:
                    self.files += [join(ply_path, f, self.room_name + '.ply')]
                    break
            else:
                raise ValueError('Unknown set for: ', self.set)
        print(self.files)
        ...
        if self.set in ['test', 'ERF']:
            ...
            self.cloud_names = [self.room_name]
        else:
            raise ValueError('Unknown set for: ', self.set)
    ...
    def prepare_room_ply(self, cloud_name):
        ...
        print('\nPreparing ply files for room ' + cloud_name + '/' + self.room_name)
        t0 = time.time()

        # Folder for the ply files
        ply_path = join(self.path, self.train_path, cloud_name)
        if not exists(ply_path):
            makedirs(ply_path)
        # Return if the cloud has already been computed
        room_file = join(ply_path, self.room_name + '.ply')
        # print(room_folder.split('/'))
        # print(ply_path)
        # print(room_file)
        if exists(room_file):
            return
        ...
        # # S3DIS
        # # Folder for original point cloud files
        # room_folder = join(self.path, cloud_name, self.room_name)
        # for object_name in listdir(join(room_folder, 'Annotations')):
        #     if object_name[-4:] == '.txt':
        #         # Text file containing point of the object
        #         object_file = join(room_folder, 'Annotations', object_name)
        ...
        # Seven Scenes
        # Folder for original point cloud files
        room_folder = join(self.path, cloud_name)
        for object_name in listdir(room_folder):
            if object_name[-4:] == '.txt':
                # Text file containing point of the object
                object_file = join(room_folder, object_name)
                # Object class and ID
                object_class = 0
                # Read object points and colors
                object_data = np.loadtxt(object_file, dtype=np.float32)
        ...
        # Save as ply
        write_ply(room_file, (room_points, room_colors, room_classes),
                  ['x', 'y', 'z', 'red', 'green', 'blue', 'class'])
...
class SinglePlySampler(Sampler):
    """Sampler for S3DIS"""

    def __init__(self, dataset: SinglePlyDataset):
        ...
        if dataset.set in ['test', 'ERF', 'visualise']:
...
class SinglePlyCustomBatch:
    """Custom batch definition with memory pinning for SingPLY"""
...
def SinglePlyCollate(batch_data):
    return SinglePlyCustomBatch(batch_data)
