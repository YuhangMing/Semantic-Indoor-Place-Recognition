...
class NYUv2TripleDataset(PointCloudDataset):
    """Class to handle NYUv2 dataset for Triple segmentation."""

    def __init__(self, config, set='training', balance_classes=True):
        PointCloudDataset.__init__(self, 'NYUv2Triple')
...
class NYUv2TripleSampler(Sampler):
    """
    Sampler for NYUv2Triple
    Generate the index for loading at each iteration/step
    """

    def __init__(self, dataset: NYUv2TripleDataset):
...
class NYUv2TripleCustomBatch:
    """
    Custom batch definition with memory pinning for NYUv2Triple
...
def NYUv2TripleCollate(batch_data):
    return NYUv2TripleCustomBatch(batch_data)
