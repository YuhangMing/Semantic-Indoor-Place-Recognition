...
class PointCloudDataset(Dataset):
    ...
    def init_labels(self):
        ...
        # int: number of known semantic classes
        self.num_classes = len(self.label_to_names)

        # list of int: stores labels in "int"
        self.label_values = np.sort([k for k, v in self.label_to_names.items()])

        # list of string: stores labels in "string"
        self.label_names = [self.label_to_names[k] for k in self.label_values]
        
        # dictionary: key as "int" label, value as "int" position index
        # keys and values are (should be) the same, since label_values is already sorted. 
        # why need this? In some dataset, label != idx
        self.label_to_idx = {l: i for i, l in enumerate(self.label_values)}
        
        # dictionary: key as "string" label, value as "int" label
    ...
    def segmentation_inputs(self,
        ...
        for block_i, block in enumerate(arch):
            ...
            # Get all blocks of the layer, stored in layer_blocks
            if not ('pool' in block or 'strided' in block or 'global' in block or 'upsample' in block):
                layer_blocks += [block]
                # print('... continue')
                continue
            ...
            if layer_blocks:
                ...
                conv_i = batch_neighbors(stacked_points, stacked_points, stack_lengths, stack_lengths, r)
                # conv_i is a NxM array, N is the stack length, i.e. num of query points, M is the number of neighbors
                # elements are the indices of the neighbor points with the first element be the query point itself
                # print(stack_lengths)
                # print(conv_i.shape)
                # print(np.max(conv_i[0, :]))
                # print(np.min(conv_i[0, :]))
            ...
            if 'pool' in block or 'strided' in block:
                ...
                # Subsample indices
                # print(pool_p.shape, stacked_points.shape, pool_b, stack_lengths, r)
                pool_i = batch_neighbors(pool_p, stacked_points, pool_b, stack_lengths, r)
                # print(pool_i.shape)
                # print(np.min(pool_i[0, :]))
                # print(np.max(pool_i[0, :]))
