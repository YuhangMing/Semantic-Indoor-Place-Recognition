...
if __name__ == '__main__':
    ...
    # Set root paths
    scannet_path = '/home/adam/Yuhang/Dataset/ScanNetPR'
    ...
    # ##########################
    # ## get test positives for quicker testing
    # # # positive and negative indices for each pcd
    # # vlad_pn_file = join(scannet_path, 'VLAD_triplets', 'vlad_pos_neg.txt')
    ...
    #         # path to pcds
    #         scene_path = join(scannet_data_path, scene)
    #         scene_pcd_path = join(scene_path, 'input_pcd')
    #         scene_pcd_0mean_path = join(scene_path, 'input_pcd_0mean')
    #         scene_pcd_pose_path = join(scene_path, 'pose')
    #         if not exists(scene_pcd_path) or not exists(scene_pcd_0mean_path):
    #             raise ValueError('Missing path:', scene_pcd_path, scene_pcd_0mean_path)
            
    #         # print(all_scene_pcds[scene])
    #         scene_cntrs = []
    #         for pcd in all_scene_pcds[scene]:
    #             # get pose
    #             frame_id = pcd[13:-8]
    #             pose = np.loadtxt(join(scene_pcd_pose_path, frame_id+'.txt'))
    #             chk_val = np.sum(pose)
    #             if np.isinf(chk_val) or np.isnan(chk_val):
    #                 raise ValueError("Invalid pose, shouldn't happen !!!!")

    #             # get pcd centroid in camera coordinate frame
    #             pcd_file = join(scene_pcd_path, pcd)
    #             data = read_ply(pcd_file)
    #             pts = np.vstack((data['x'], data['y'], data['z'])).astype(np.float32).T
    #             if pts.shape[0] < 2:
    #                 raise ValueError("Empty Polygan Mesh !!!!")
    #             cntr = np.mean(pts, axis=0)

    #             # get centroid in world coordinate frame
    #             cntr = pose[:3,:3] @ cntr + pose[:3, 3]
    #             scene_cntrs.append(cntr)

    #         # double loop to compute distance
    #         scene_test_positives = []
    #         for j, cntr_q in enumerate(scene_cntrs):
    #             test_positives = []
    #             for k, cntr_r in enumerate(scene_cntrs):
    #                 if j == k:
    #                     continue
    #                 # compute distance 
    #                 dist = np.linalg.norm(cntr_q - cntr_r)
    #                 if dist <= test_threshold:
    #                     test_positives.append(k)
    #             scene_test_positives.append(test_positives)
    #         all_scene_test_pos[scene] = scene_test_positives
    #     #     # print(scene_test_positives[0])
    #     #     # print(all_scene_pos_neg[scene][0])
    #     #     break
    #     # break

    # print(len(all_scene_test_pos))
    
    # with open(join(scannet_path, 'vlad_test_pos.txt'), 'wb') as vlad_file:
    #     pickle.dump(all_scene_test_pos, vlad_file)
    # print('VLAD saved to', join(scannet_path, 'vlad_test_pos.txt\n'))
    

    ######################
    ## Generate pos-neg pairs for training
    # get non_zero_meaned pcd path and zero_meaned pcd path 
    pcd_prev_saved_path = join(scannet_data_path, 'input_pcd')
    if not exists(pcd_prev_saved_path):
        os.makedirs(pcd_prev_saved_path)
    pcd_new_path = join(scannet_data_path, 'input_pcd_0mean')
    if not exists(pcd_new_path):
    ...
    for task in tasks:
        ...
        for j, scene in enumerate(scenes):
            ...
            scannet_scene_path = join(scannet_data_path, scene)
            # path to previously stored submap, non-zero-meaned and at 2 FPS
            scene_pcd_stored_path = join(pcd_prev_saved_path, scene)
            # path to store new submaps, spatially sparser 
            scene_pcd_new_path = join(pcd_new_path, scene)
            if not exists(scene_pcd_new_path):
                os.makedirs(scene_pcd_new_path)

            # get num of frames and the intrinsic matrix
            K = []
            nFrames = 0
            info_file = open(join(scannet_scene_path, scene+'.txt'))
            ...
            # hd_scene_pcd = o3d.io.read_point_cloud(join(scannet_scene_path, scene+'_vh_clean.ply'))
            ...
            for i in range(0, nFrames, stepFrm):
                ...
                pose = np.loadtxt(join(scannet_scene_path, 'pose', str(i)+'.txt'))
                chk_val = np.sum(pose)
                if np.isinf(chk_val) or np.isnan(chk_val):
                    continue

                # get subsampled point cloud
                sub_pcd_file = join(scene_pcd_stored_path, file_name)
                ...
                if save_pcd_to_file:
                    ...
                    new_sub_pcd_file = join(scene_pcd_new_path, file_name)
    ...
    with open(join(scannet_path, 'vlad_pcd.txt'), 'wb') as pcd_file:
        pickle.dump(all_files, pcd_file)
    print('\nPCD saved to', join(scannet_path, 'vlad_pcd.txt'))
    with open(join(scannet_path, 'vlad_pos_neg.txt'), 'wb') as vlad_file:
        pickle.dump(all_scene_pos_neg, vlad_file)
    print('VLAD saved to', join(scannet_path, 'vlad_pos_neg.txt\n'))

    #### display test
    print('Test with loading')
    with open(join(scannet_path, 'vlad_pos_neg.txt'), 'rb') as f:
    ...
    with open(join(scannet_path, 'vlad_pcd.txt'), 'rb') as f:
    ...
    path_to_pcds = join(scannet_data_path, 'input_new_pcd')
    ...
    for sId, scene in enumerate(scene_list):
        ...
        scene_path = join(path_to_pcds, scene)
    ...
    with open(join(scannet_path, 'pcd_list.txt'), 'wb') as pcd_file:
